...
def adminsearch():
    ...
    if category=="location":
        result=db.session.query(Parking_lot).filter(Parking_lot.Address.ilike(f"%{search_value}%"))
        parking=db.session.query(ParkingHistory).all()
        latest_status={}
        for record in parking:
            key=(record.lot_id,record.spot_id)
            latest_status[key]=record.status
        return render_template("searchparkinglot.html",parking_lot=result,spot_status=latest_status)
    if category=="lot_name":
        result=db.session.query(Parking_lot).filter(Parking_lot.Name.ilike(f"%{search_value}%"))
        parking=db.session.query(ParkingHistory).all()
        latest_status={}
        for record in parking:
            key=(record.lot_id,record.spot_id)
